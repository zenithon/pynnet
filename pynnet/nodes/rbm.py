...
class RBMNode(BaseNode):
    r"""
    Restricted Boltzmann Machine node.

    Examples:
    >>> r = RBMNode(10, 8)
    >>> r = RBMNode(12, 12, sampled=True)
    >>> r = RBMNode(4, 3, persistent=True)
    ...
    def build(self, input, input_shape=None):
        ...
        Tests:
        >>> r = RBMNode(3, 2, sampled=False, dtype='float32')
        >>> x = T.fmatrix('x')
        ...
        >>> r = RBMNode(3, 2, sampled=True, dtype='float32')
        >>> x = T.fmatrix('x')
        >>> r.build(x, input_shape=(4, 3))
        >>> r.output_shape
        (4, 2)
